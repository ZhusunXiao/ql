...
# ===== Helper Functions =====
def extract_function_name(line, match=None):
        ...
        "rules": [
            {
                ...
                # Use lambda to dynamically generate cursor
                "cursor": lambda line, match: "DOMAIN_DEINIT_" + (
                    re.search(r'domain (\d+)', line).group(1)
                    if re.search(r'domain (\d+)', line) else "X"
                ),
                "layer": 3
            }
        ]
