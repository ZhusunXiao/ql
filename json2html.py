...
<body>
            ...
            <div class="content">
                <div class="toolbar">
                    ...
                    <div class="toolbar-group">
                        ...
                        <button class="toolbar-btn" id="add-vline">📍 Add VLine</button>
    ...
    <div class="annotation-panel" id="annotation-panel">
        ...
        <div style="margin-bottom: 8px;">
            <span style="font-size: 12px; color: #666; margin-right: 10px;">Quick Fill:</span>
            <button type="button" id="fill-cursor" style="padding: 4px 10px; font-size: 11px; background: #6c757d; color: white; border: none; border-radius: 4px; cursor: pointer; margin-right: 5px;" title="Fill with cursor name">🎯 Cursor</button>
            <button type="button" id="fill-msg" style="padding: 4px 10px; font-size: 11px; background: #6c757d; color: white; border: none; border-radius: 4px; cursor: pointer;" title="Fill with message">💬 Msg</button>
        </div>
    ...
    <!-- Vertical Line Panel -->
    <div class="overlay" id="vline-overlay"></div>
    <div class="annotation-panel" id="vline-panel">
        <h3>📍 Add Vertical Line</h3>
        <div id="vline-time-info" style="margin-bottom: 10px; font-size: 12px; color: #666;"></div>
        <div style="margin-bottom: 10px;">
            <label style="font-size: 12px; color: #666; margin-right: 10px;">Line Color:</label>
            <input type="color" id="vline-color" value="#ff6b6b" style="width: 60px; height: 28px; border: none; cursor: pointer;">
            <span style="margin-left: 10px;">
                <span class="color-preset vline-color-preset" data-color="#ff6b6b" style="display:inline-block;width:20px;height:20px;background:#ff6b6b;border-radius:3px;cursor:pointer;margin-right:5px;vertical-align:middle;"></span>
                <span class="color-preset vline-color-preset" data-color="#4ecdc4" style="display:inline-block;width:20px;height:20px;background:#4ecdc4;border-radius:3px;cursor:pointer;margin-right:5px;vertical-align:middle;"></span>
                <span class="color-preset vline-color-preset" data-color="#ffe66d" style="display:inline-block;width:20px;height:20px;background:#ffe66d;border-radius:3px;cursor:pointer;margin-right:5px;vertical-align:middle;"></span>
                <span class="color-preset vline-color-preset" data-color="#95e1d3" style="display:inline-block;width:20px;height:20px;background:#95e1d3;border-radius:3px;cursor:pointer;margin-right:5px;vertical-align:middle;"></span>
                <span class="color-preset vline-color-preset" data-color="#a29bfe" style="display:inline-block;width:20px;height:20px;background:#a29bfe;border-radius:3px;cursor:pointer;vertical-align:middle;"></span>
            </span>
        </div>
        <div style="margin-bottom: 10px;">
            <label style="font-size: 12px; color: #666;">Label (optional, defaults to time):</label>
        </div>
        <input type="text" id="vline-text" placeholder="Leave empty for time label..." style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 5px; margin-bottom: 10px; box-sizing: border-box;">
        <div class="btn-group">
            <button class="btn-delete" id="vline-delete" style="display:none;">Delete</button>
            <button class="btn-cancel" id="vline-cancel">Cancel</button>
            <button class="btn-save" id="vline-save">Save</button>
        </div>
    </div>
    ...
    <script>
        ...
        function generateSidebar() {{
            ...
            classHierarchy.forEach((cls, classIndex) => {{
                ...
                // Check if all subclasses are visible
                const allVisible = cls.subclasses.every(sub => visibleSubclasses.has(sub.category_label));
                const toggleIcon = allVisible ? '✓' : '✗';
                const toggleTitle = allVisible ? 'Hide all subclasses on chart' : 'Show all subclasses on chart';
                ...
                html += `<span class="class-visibility-toggle" data-class-index="${{classIndex}}" title="${{toggleTitle}}">${{toggleIcon}}</span>`;
            ...
            document.querySelectorAll('.subclass-item input[type="checkbox"]').forEach(checkbox => {{
                checkbox.addEventListener('change', function() {{
                    ...
                    updateChart();
                    // Update toggle icon for this class
                    updateClassToggleIcons();
                }});
            }});
            
            // Bind class visibility toggle events
            document.querySelectorAll('.class-visibility-toggle').forEach(toggle => {{
                toggle.addEventListener('click', function(e) {{
                    e.stopPropagation(); // Prevent triggering class collapse
                    const classIndex = parseInt(this.dataset.classIndex);
                    const cls = classHierarchy[classIndex];
                    const allVisible = cls.subclasses.every(sub => visibleSubclasses.has(sub.category_label));
                    
                    // Toggle all subclasses visibility
                    cls.subclasses.forEach(sub => {{
                        const checkbox = document.querySelector(`input[data-category="${{sub.category_label}}"]`);
                        if (allVisible) {{
                            visibleSubclasses.delete(sub.category_label);
        ...
        // Update class toggle icons based on visibility state
        function updateClassToggleIcons() {{
            document.querySelectorAll('.class-visibility-toggle').forEach(toggle => {{
                const classIndex = parseInt(toggle.dataset.classIndex);
                const cls = classHierarchy[classIndex];
                const allVisible = cls.subclasses.every(sub => visibleSubclasses.has(sub.category_label));
                toggle.textContent = allVisible ? '✓' : '✗';
                toggle.title = allVisible ? 'Hide all subclasses on chart' : 'Show all subclasses on chart';
            }});
        }}
        ...
        const vlineStorageKey = 'timeline-vlines-{file_name}';
        let vlines = {{}};
        let vlineMode = false;
        let currentVlineTime = null;
        let currentVlineKey = null;
        
        // Load vertical lines from localStorage
        function loadVlines() {{
            try {{
                const stored = localStorage.getItem(vlineStorageKey);
                if (stored) {{
                    vlines = JSON.parse(stored);
                }}
            }} catch (e) {{
                console.warn('Failed to load vlines:', e);
                vlines = {{}};
            }}
        }}
        
        // Save vertical lines to localStorage
        function saveVlines() {{
            try {{
                localStorage.setItem(vlineStorageKey, JSON.stringify(vlines));
            }} catch (e) {{
                console.warn('Failed to save vlines:', e);
            }}
        }}
        ...
        // Show annotations on chart (includes both point annotations and vertical lines)
        function renderAnnotations() {{
            ...
            if (showAnnotations && Object.keys(vlines).length > 0) {{
                try {{
                    ...
                    if (gridModel && gridModel.coordinateSystem) {{
                        ...
                        Object.entries(vlines).forEach(([key, vline]) => {{
                            const pixel = myChart.convertToPixel({{xAxisIndex: 0}}, vline.time);
                            
                            if (pixel === undefined || isNaN(pixel)) return;
        ...
        function openAnnotationPanel(point, isEdit) {{
            ...
            // Get point's layer color
            const pointColor = getLayerColor(point.layer);
            
            document.getElementById('annotation-point-info').innerHTML = 
                '<strong>' + point.classname + '</strong> / ' + point.subclassname + '<br>' +
                '<span style="color:' + pointColor + '">' + formatTime(point.value[0]) + '</span> - Layer ' + point.layer;
            
            document.getElementById('annotation-text').value = existingAnnotation ? existingAnnotation.text : '';
            // Use existing annotation color, or point's layer color as default
            document.getElementById('annotation-color').value = existingAnnotation ? (existingAnnotation.color || pointColor) : pointColor;
        ...
        // Quick fill cursor and auto-save
        document.getElementById('fill-cursor').addEventListener('click', function() {{
            if (currentAnnotationPoint) {{
                const text = currentAnnotationPoint.cursor || '';
                if (text) {{
                    const color = document.getElementById('annotation-color').value;
                    const key = getAnnotationKey(currentAnnotationPoint);
                    annotations[key] = {{
        ...
        // Quick fill message and auto-save
        document.getElementById('fill-msg').addEventListener('click', function() {{
            if (currentAnnotationPoint) {{
                const text = currentAnnotationPoint.msg || '';
                if (text) {{
                    const color = document.getElementById('annotation-color').value;
                    const key = getAnnotationKey(currentAnnotationPoint);
                    annotations[key] = {{
            ...
            reader.onload = function(event) {{
                try {{
                    ...
                    if (imported.annotations) {{
                        ...
                        saveAnnotations();
                        renderAnnotations();
                        importCount += Object.keys(imported.annotations).length;
                    }}
                    if (imported.vlines) {{
                        // Merge imported vlines
                        Object.assign(vlines, imported.vlines);
                        saveVlines();
                        renderAnnotations();
                        importCount += Object.keys(imported.vlines).length;
                    }}
        ...
        // Load vertical lines (rendered together with annotations)
        loadVlines();
        
        // Open vline panel
        function openVlinePanel(time, existingKey) {{
            currentVlineTime = time;
            currentVlineKey = existingKey;
            
            const existingVline = existingKey ? vlines[existingKey] : null;
            const displayTime = (typeof time === 'number' && !isNaN(time)) ? formatTime(time) : 'Unknown';
            
            document.getElementById('vline-time-info').innerHTML = 
                '<strong>Time:</strong> <span style=\"color:#ff6b6b\">' + displayTime + '</span>';
            
            document.getElementById('vline-text').value = existingVline ? (existingVline.text || '') : '';
            document.getElementById('vline-color').value = existingVline ? (existingVline.color || '#ff6b6b') : '#ff6b6b';
            document.getElementById('vline-delete').style.display = existingVline ? 'inline-block' : 'none';
            
            document.getElementById('vline-overlay').style.display = 'block';
            document.getElementById('vline-panel').style.display = 'block';
            document.getElementById('vline-text').focus();
        }}
        
        // Close vline panel
        function closeVlinePanel() {{
            document.getElementById('vline-overlay').style.display = 'none';
            document.getElementById('vline-panel').style.display = 'none';
            currentVlineTime = null;
            currentVlineKey = null;
        }}
        
        // VLine mode toggle
        document.getElementById('add-vline').addEventListener('click', function() {{
            vlineMode = !vlineMode;
            this.classList.toggle('active', vlineMode);
            this.textContent = vlineMode ? '📍 Click Chart...' : '📍 Add VLine';
            if (vlineMode) {{
        ...
        myChart.getZr().on('click', function(params) {{
            ...
            if (myChart.containPixel('grid', pointInPixel)) {{
                ...
                const dataPoint = myChart.convertFromPixel({{xAxisIndex: 0, yAxisIndex: 0}}, pointInPixel);
                const xValue = dataPoint[0];  // X-axis time value
                
                // Validate xValue is a valid number
                if (typeof xValue !== 'number' || isNaN(xValue)) {{
                    console.warn('Invalid click time:', xValue, dataPoint);
                    return;
                }}
                
                // Exit vline mode
                vlineMode = false;
                document.getElementById('add-vline').classList.remove('active');
                document.getElementById('add-vline').textContent = '📍 Add VLine';
                chartDom.style.cursor = 'default';
                
                // Open panel
                openVlinePanel(xValue, null);
            }}
        }});
        
        // Save vline
        document.getElementById('vline-save').addEventListener('click', function() {{
            if (currentVlineTime === null) return;
            
            const text = document.getElementById('vline-text').value.trim();
            const color = document.getElementById('vline-color').value;
            const key = currentVlineKey || ('vline_' + currentVlineTime);
            
            vlines[key] = {{
                time: currentVlineTime,
                text: text || null,  // null means use default time label
                color: color,
                createdAt: new Date().toISOString()
            }};
        ...
        // Cancel vline
        document.getElementById('vline-cancel').addEventListener('click', closeVlinePanel);
        document.getElementById('vline-overlay').addEventListener('click', closeVlinePanel);
        
        // Delete vline
        document.getElementById('vline-delete').addEventListener('click', function() {{
            if (currentVlineKey && vlines[currentVlineKey]) {{
                delete vlines[currentVlineKey];
                saveVlines();
                renderAnnotations();
            }}
            closeVlinePanel();
        ...
        // VLine color presets
        document.querySelectorAll('.vline-color-preset').forEach(preset => {{
            preset.addEventListener('click', function() {{
                document.getElementById('vline-color').value = this.dataset.color;
            }});
        }});
